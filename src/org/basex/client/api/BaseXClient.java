...
public final class BaseXClient {
	 ...
	 *             I/O exception
	 */
	public void createCol(final String name) throws IOException {
		sendCreateCol(8, name);
	}
	 ...
	 *             I/O exception
	 */
	void receive(final InputStream is, final OutputStream os)
			throws IOException {
	 ...
	 *             I/O exception
	 */
	private void addPrepare(final int cmd, final String path)
			throws IOException {
		out.write(cmd);
		send(path);
	}
	 ...
	 *             I/O exception
	 */
	private void sendCreateCol(final int cmd, final String path)
			throws IOException {
		out.write(cmd);
		send(path);
		sendCol();
	...
	public final class Query {
		 ...
		 *             I/O Exception
		 */
		public void execute(final OutputStream o) throws IOException {
			out.write(5);
			send(id);
			receive(in, o);
			if (!ok())
